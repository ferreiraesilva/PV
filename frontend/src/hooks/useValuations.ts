...
export const useValuations = () => {
  ...
  const updateCashflow = useCallback((index: number, key: keyof CashflowRow, value: string | number) => {
    setCashflows(
      produce(draft => {
  ...
  const updateScenario = useCallback((index: number, key: keyof ScenarioRow, value: string | number) => {
    setScenarios(
      produce(draft => {
        (draft[index] as any)[key] = value;
      }),
