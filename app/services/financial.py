...
def present_value(schedule: Sequence[tuple[int, float]], discount_rate: float, periods_per_year: int = 12) -> float:
    rate_per_period = discount_rate / periods_per_year
    return sum(amount / ((1 + rate_per_period) ** period) for period, amount in schedule)


def payment(principal: float, discount_rate: float, periods: int, periods_per_year: int = 12) -> float:
    rate_per_period = discount_rate / periods_per_year
    if rate_per_period == 0:
        return principal / periods
    factor = (rate_per_period * (1 + rate_per_period) ** periods) / ((1 + rate_per_period) ** periods - 1)
    return principal * factor


def future_value(principal: float, discount_rate: float, periods: int, periods_per_year: int = 12) -> float:
    rate_per_period = discount_rate / periods_per_year
...
def calculate_simulation_metrics(principal: float, discount_rate: float, installments: List[tuple[int, float]], periods_per_year: int = 12) -> dict[str, float]:
...
def calculate_portfolio_value(cashflows: Iterable[Cashflow], discount_rate: float, default_multiplier: float = 1.0, cancellation_multiplier: float = 1.0, periods_per_year: int = 12) -> dict[str, float]:
    ...
    for idx, cashflow in enumerate(sorted_cashflows, start=1):
        rate_per_period = discount_rate / periods_per_year
        probability_default = min(max(cashflow.probability_default * default_multiplier, 0.0), 1.0)
        probability_cancellation = min(max(cashflow.probability_cancellation * cancellation_multiplier, 0.0), 1.0)
