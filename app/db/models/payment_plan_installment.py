...
class PaymentPlanInstallment(Base):
    ...
    __table_args__ = (
        UniqueConstraint("template_id", "period", name="uq_payment_plan_installments_period"),
    )

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    template_id = Column(UUID(as_uuid=True), ForeignKey("payment_plan_templates.id", ondelete="CASCADE"), nullable=False)
