...
from app.services.financial_settings import FinancialSettingsService

# Routers
superuser_router = APIRouter(
    prefix="/superuser",
    tags=["Admin Portal (Superuser)"],
    dependencies=[Depends(require_roles(SUPERADMIN_ROLE))],
)
tenant_admin_router = APIRouter(
    prefix="/tenant-admin",
    tags=["Admin Portal (Tenant Admin)"],
...
def get_financial_settings(
    request: Request,
    tenant_id: Annotated[str, Path(pattern=r"^[0-9a-fA-F-]{36}$")],
    service: FinancialSettingsServiceDependency,
    ...
    if not (acting.is_superuser() or acting.is_tenant_admin_for(target_tenant_id)):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Insufficient permissions for tenant financial settings",
        )
    try:
        settings = service.get_financial_settings(target_tenant_id)
    except Exception as exc:
        _handle_service_error(exc)
...
def update_financial_settings(
    ...
    try:
        settings = service.update_financial_settings(acting, target_tenant_id, updates)
    except Exception as exc:
        _handle_service_error(exc)
    response = _financial_settings_response(settings)
    _audit_entity(
