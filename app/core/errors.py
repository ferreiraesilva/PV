...
def _build_payload(
    ...
    return ErrorResponse(
        code=code, message=message, detail=detail, request_id=request_id
    )


def _log_error(
    request: Request, *, code: str, message: str, status_code: int, detail: Any | None
) -> None:
    logger.bind(
        component="api", request_id=getattr(request.state, "request_id", None)
    ).error(
        {
            "error_code": code,
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def handle_app_exception(request: Request, exc: AppException) -> JSONResponse:
        ...
        payload = _build_payload(
            request, code=exc.code, message=exc.message, detail=exc.detail
        )
